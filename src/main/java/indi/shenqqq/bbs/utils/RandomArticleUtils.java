 ...
 * @Description XX
 */
public class RandomArticleUtils {

    public static String generatorRandomArticle(String title, int length) {
        String content = generator(title, length);
        return content;
    }

    private static int randomNum(int mix, int max) {
        int num = (int) (Math.random() * (max - mix) + mix);
        return num;
    }

    private static String generator(String title, Integer length) {
        if (length == null) length = 600;
        String body = "";
        while (body.length() < length) {
            ...
            if (num < 10) {
                ...
                body += getRandomChoice(famous)
                        .replace("a", getRandomChoice(before))
                        .replace("b", getRandomChoice(after));
            } else {
                body += getRandomChoice(bosh);
    ...
    private static String getRandomChoice(String[] list) {
