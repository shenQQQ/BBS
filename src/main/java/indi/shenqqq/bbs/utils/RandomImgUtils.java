...
public class RandomImgUtils {
    ...
    public static final String cache_url="D:\\JAVA\\BBS\\static\\upload\\";

    public static String getTitleImg(String context,int width,int height,String filename){
        //1.在内存中创建一张图片
        ...
        //String random = drawRandomNum((Graphics2D) g, "ch");//根据客户端传递的createTypeFlag标识生成验证码图片
        drawContent((Graphics2D) g,context,width,height);
        try {
            ImageIO.write(bi, "jpeg", new File(cache_url + filename));
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    private static void setBorder(Graphics g,int width ,int height) {
        ...
        // 边框区域
        g.drawRect(1, 1, width - 2, width - 2);
    }
    ...
    private static void drawContent(Graphics2D g, String content,int width,int height){
        // 设置颜色
        ...
        String ch ="";
        g.drawString(content,width/2-content.length()*15,height/2-15);
...
//            sb.append(ch);
//            // 正向角度
//            g.rotate(degree * Math.PI / 180, 0, 0);
//            g.drawString(ch, x+i*30, height/2);
//            // 反向角度
//            g.rotate(-degree * Math.PI / 180, 0, 0);
//        }
    }
